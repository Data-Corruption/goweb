...
func (m *DaemonManager) Start(ctx context.Context) error {
	...
	return m.start_internal(ctx, selfPath)
}

// start_internal launches the application as a daemon.
// It's split up like this to allow concurrent testing.
func (m *DaemonManager) start_internal(ctx context.Context, binPath string) error {
...
func (m *DaemonManager) Stop(ctx context.Context) error {
	...
	if !isOurBinary(pid) {
		fmt.Println("Stored PID does not match the current binary. Not stopping.")
		xlog.Errorf(ctx, "Process with PID %d is not running the expected binary. Not stopping", pid)
		return m.writePID(0)
	}

	return m.stop(ctx, process)
}
...
func (m *DaemonManager) Restart(ctx context.Context) error {
	...
	// sleep 500ms so os/fs can catch it's breath
...
// stop shuts down the daemon process.
// Starts with a graceful SIGTERM and escalates to SIGKILL if exceeds the timeout.
